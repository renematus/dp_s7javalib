...
*/
package droid.vsb.dp.rmatu.s7javalib.libnodave_orig;
public class PDU {
	/*
	typedef struct {
	    uc P;	// allways 0x32
	...
	*/
	...
	public int addVarToReadRequest(int area, int DBnum, int start, int len) {
		...
		if ((area == Nodave.ANALOGINPUTS200)
			|| (area == Nodave.ANALOGOUTPUTS200)) {
			pa[3] = 4;
			start *= 8; /* bits */
		} else if (
			(area == Nodave.TIMER)
				|| (area == Nodave.COUNTER)
				|| (area == Nodave.TIMER200)
	...
	public int addBitVarToReadRequest(
		int area,
		int DBnum,
		int start,
		int len) {
...
*/
	...
	*/
	void addValue(byte[] values) {
		...
		if (mem[data + 1] == 4) { // bit data, length is in bits
			...
			if ((Nodave.Debug & Nodave.DEBUG_PDU) != 0)
				System.out.println(
					"unknown data type/length: " + mem[data + 1]);
		}
		if (udata == 0)
			udata = data + 4;
		udlen += values.length;
	...
	int testResultData() {
		...
		if ((mem[data] == (byte)255) && (dlen > 4)) {
			...
			if (mem[data + 1] == 4) {
				...
				if ((Nodave.Debug & Nodave.DEBUG_PDU) != 0)
					System.out.println(
						"fixme: what to do with data type " + mem[data + 1]);
	...
	*/
	...
	public void addVarToWriteRequest(
		int area,
		int DBnum,
		int start,
		int byteCount,
